...
function pagePilling(){
    $('#pagepiling').pagepiling({
        ...
        anchors: ['hero', 'about', 'resume', 'services', 'portfolio', 'contact', 'footer'],
        menu: '#myMenu',
        direction: 'vertical',
        verticalCentered: true,
        navigation: {
            'position': 'right',
            'tooltips': ['HOME', 'ABOUT ME', 'RESUME','SERVICES','PORTFOLIO', 'CONTACT', 'FOOTER']
        },
        loopBottom: true,
        loopTop: true,
        scrollingSpeed: 700,
...
function sendEmail() {
    ...
    if(!name){
        ...
        $.ajax({
            ...
            url:  "file:///D:/xampp/htdocs/TabaiAlaEddin.github.io/sendEmail.php",
            beforeSend: function() {
            ...
            error: function(xhr) {
                $('#submit-btn').html('Submit');
                $('#message').toast('show').addClass('bg-danger').removeClass('bg-success bg-warning');
                $('.toast-body').html('<strong> Error : </strong> Something went wrong, Please try again or use the mail link .');
            },
        });
    }
}